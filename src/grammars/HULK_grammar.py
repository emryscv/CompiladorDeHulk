...
expr %= expr + at + stringify, lambda h, s: BinaryOperationNode(s[1], s[3], s[2])
...
expr %= let + var_def + in_token + expr, lambda h, s: LetInNode(s[2], s[4])
expr %= id + asign + expr, lambda h, s: VarReAsignNode(s[1], s[3])
